...
<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ec2="http://ec2.amazonaws.com/doc/2009-07-15/"
	exclude-result-prefixes="ec2">
	<xsl:output method="xml" omit-xml-declaration="no" indent="yes" />
	<xsl:variable name="ns"
		select="'http://ec2.amazonaws.com/doc/2009-07-15/'" />
	...
	<xsl:template match="ec2:attachment">
		<xsl:element name="VpcAttachment" namespace="{$ns}">
			<xsl:element name="VpcId" namespace="{$ns}">
				<xsl:value-of select="ec2:attachment/vpcId" />
			</xsl:element>
			<xsl:element name="AttachmentState" namespace="{$ns}">
